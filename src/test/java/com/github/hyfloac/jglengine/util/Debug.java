...
public class Debug
{
    ...
    public static void debugTexture(int textureID) { Renderer.INSTANCE.add(new GameObject(new Model(getTextureTestModel()).setTexID(textureID), new Vector3f(0.0f, 0.0f, -3.0f))); }
    ...
    public static void addDebugObjects(int radius, int numObjects, boolean rotate)
    {
        ...
        for(int i = 0; i < numObjects; ++i)
        {
            ...
            Renderer.INSTANCE.add(new GameObject(new Model(Models.CUBE).setTexID(Textures.CUBE.texID), new Vector3f(x, y, z), new Vector3f(rx, ry, rz)));
        }
    }
    ...
    public static void test()
    {
        ...
        for(int i = 0; i < len; ++i)
        {
            ...
            Renderer.INSTANCE.add(new GameObject(new Model(Models.CUBE).setTexID(Textures.CUBE.texID), pos, new Vector3f(0.0f, (float) (x * z * 120), 0.0f)));
        }
        rad *= 2;
        Renderer.INSTANCE.add(new GameObject(new Model(Models.CUBE).setTexID(Textures.CUBE.texID), new Vector3f(rad, rad, rad), new Vector3f(0, 0, 0)));
        Renderer.INSTANCE.add(new GameObject(new Model(Models.CUBE).setTexID(Textures.CUBE.texID), new Vector3f(rad, rad, -rad), new Vector3f(0, 0, 0)));
        Renderer.INSTANCE.add(new GameObject(new Model(Models.CUBE).setTexID(Textures.CUBE.texID), new Vector3f(rad, -rad, rad), new Vector3f(0, 0, 0)));
        Renderer.INSTANCE.add(new GameObject(new Model(Models.CUBE).setTexID(Textures.CUBE.texID), new Vector3f(rad, -rad, -rad), new Vector3f(0, 0, 0)));
        Renderer.INSTANCE.add(new GameObject(new Model(Models.CUBE).setTexID(Textures.CUBE.texID), new Vector3f(-rad, rad, rad), new Vector3f(0, 0, 0)));
