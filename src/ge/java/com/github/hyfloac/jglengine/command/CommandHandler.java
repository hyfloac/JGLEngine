...
import com.github.hyfloac.jglengine.command.commands.*;
...
public class CommandHandler
{
    ...
    static
    {
        commands = ListAdapter.adapt(Lists.newArrayList(new GLCommand(),
                                                        new ExitCommand(),
                                                        new RenderLiveCommand(),
                                                        new LogCommand(),
                                                        new DebugCommand(),
                                                        new AliasCommand())).toImmutable();

        String s = ResourceHelper.getGameDataDir();
        File f = new File(s, "autoexec.cfg");
        if(f.exists())
        {
            try
            {
                BufferedReader br = IO.newBufReader(new FileInputStream(f));
                String line;
    ...
    public static void parse(String in)
    {
        ...
        String[] split = StringUtil.SPACE_PATTERN.split(in);
        for(Command c : commands)
        {
            if(Command.equals(split[0], c.aliases))
            {
                String[] args;
                if(split.length > 1)
                {
