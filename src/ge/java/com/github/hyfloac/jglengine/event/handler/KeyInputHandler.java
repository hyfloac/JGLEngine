...
public class KeyInputHandler
{
    ...
    private final KeyCheck fpsLogging;
    private final KeyCheck x0;
    private final KeyCheck x1;
    private final KeyCheck y0;
    private final KeyCheck y1;

    public KeyInputHandler()
    {
        quit = new KeyCheck(GLFW.GLFW_KEY_ESCAPE, GLFW.GLFW_RELEASE, 0);
        toggleDebug = new KeyCheck(GLFW.GLFW_KEY_8, GLFW.GLFW_PRESS, 0);
        ...
        fpsLogging = new KeyCheck(GLFW.GLFW_KEY_Z, GLFW.GLFW_PRESS, 0);
        x0 = new KeyCheck(GLFW.GLFW_KEY_M, GLFW.GLFW_PRESS, 0);
        x1 = new KeyCheck(GLFW.GLFW_KEY_N, GLFW.GLFW_PRESS, 0);
        y0 = new KeyCheck(GLFW.GLFW_KEY_B, GLFW.GLFW_PRESS, 0);
        y1 = new KeyCheck(GLFW.GLFW_KEY_V, GLFW.GLFW_PRESS, 0);
    }
    ...
    public static final class KeyCheck
    {
        ...
        public KeyCheck(int keyCheck, int scanCodeCheck, int actionCheck, int modsCheck) { this(new int[] { keyCheck }, scanCodeCheck, actionCheck, new int[] { modsCheck }); }
        ...
        public KeyCheck(int[] keyCheck, int actionCheck) { this(keyCheck, -1, actionCheck, new int[] { -1 }); }

        public KeyCheck(int keyCheck) { this(keyCheck, -1, -1, -1); }

        public KeyCheck(int[] keyCheck) { this(keyCheck, -1, -1, new int[] { -1 }); }

        public KeyCheck(EventKeyInput event) { this(event.key, event.scancode, event.action, event.mods); }

        public boolean check(EventKeyInput event)
        {
            for(int key : keyCheck)
            {
                if(!(event.key == key || key == -1)) { return false; }
            }

            for(int mods : modsCheck)
            {
                if(!(event.mods == mods || mods == -1)) { return false; }
            }

            return (scanCodeCheck == -1 || scanCodeCheck == event.scancode) && (actionCheck == -1 || actionCheck == event.action);
        }
    }
}
