...
public final class ResourceHelper
{
    ...
    {
        try { return ResourceUtil.getResource("/assets" + resource); }
    ...
    public static String getShaderAsString(String resource)
    {
        try
        {
            Scanner scanner = new Scanner(getResource("/assets/shaders" + resource), "UTF-8");
    ...
    public static ByteBuffer getBufferedResource(InputStream resource) { return null; }
    ...
    {
        if(!MathUtil.isPowerOfTwo(size)) { Logger.INSTANCE.warn("Texture: `" + resource + "` has a size which is not a power of 2."); }
